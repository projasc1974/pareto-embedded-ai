/*
 * main_mlr_figuras.c - Implementación MLR para ESP32-S3
...
#include "rlm_weights.h"  // MLR = RLM (Regresión Logística Multinomial)

static const char* TAG = "MLR_PAPER";

// ============================================================================
// CONSTANTES (tomadas del archivo de pesos)
// ============================================================================
#ifndef RLM_CLASSES
...
// ============================================================================
// FUNCIONES MATEMÁTICAS (idénticas a las de CNN)
// ============================================================================
...
void matrix_vector_mult_mlr(const float* W, int rows, int cols, 
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            // W está organizado como [rows][cols] en el archivo
            y[i] += W[i * cols + j] * x[j];
        }
    }
}
...
// ============================================================================
// MODELO MLR (Regresión Logística Multinomial)
// ============================================================================

int mlr_inference(const float* input) {
    // Producto matriz-vector: W [3×784] × x [784] + bias
    // Los pesos están en rlm_weights[3][784] (del archivo)
    // IMPORTANTE: rlm_weights está organizado como [clases][features]
...
void inference_task(void *pvParameters) {
    ESP_LOGI(TAG, "\n========================================");
    ESP_LOGI(TAG, "MLR - IMPLEMENTACIÓN PAPER (SIN MAPEO)");
    ESP_LOGI(TAG, "========================================\n");
    ...
    ESP_LOGI(TAG, "Detalle: W[%d][%d] + b[%d]", RLM_CLASSES, RLM_FEATURES, RLM_CLASSES);
    ...
    ESP_LOGI(TAG, "\n========================================");
    ESP_LOGI(TAG, "RESULTADOS FINALES - MLR SIN MAPEO");
    ESP_LOGI(TAG, "========================================");
