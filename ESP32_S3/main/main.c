...
static const char *TAG = "FIGURES_MLR_NAIVE";
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "FIGURES MLR naive | test=%d | warmup=%d | measured=%d",
    ...
    const size_t params_bytes =
        sizeof(rlm_weights) + sizeof(rlm_biases);

    // buffers: logits/probs
